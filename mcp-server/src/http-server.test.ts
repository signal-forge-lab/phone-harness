...
test("HTTP boundary publishes OAuth protected-resource metadata and rejects unauthenticated MCP", async (t) => {
  ...
  const tunnelMetadata = await fetch(`${fixture.base}/.well-known/oauth-protected-resource/v1/mcp/tunnel-test`);
  assert.equal(tunnelMetadata.status, 404);
  ...
  assert.match(
    ...
    /resource_metadata="https:\/\/phone\.example\.test:8443\/\.well-known\/oauth-protected-resource\/mcp"/i,
  );
});
